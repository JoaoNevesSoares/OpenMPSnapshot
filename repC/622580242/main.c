...
{
	unsigned short bfType;        // BMַ, bmpʽļ:0x4d42(19778)
	unsigned int   bfSize;        // ļС,λ:ֽ(2-5ֽ)
	unsigned short bfReserved1;   // ,0(6-7ֽ)
	unsigned short bfReserved2;   // ,0(8-9ֽ)
	unsigned int   bfOffBits;     // ļͷݵƫ(10-13ֽ)
} BITMAPFILEHEADER;

/* ͼϢͷṹ */
typedef struct tagBITMAPINFOHEADER
{
	unsigned int    biSize;          // ˽ṹĴС(14-17ֽ)
	unsigned int    biWidth;         // ͼĿ(18-21ֽ)
	unsigned int    biHeight;        // ͼĸ(22-25ֽ)
	unsigned short  biPlanes;        // bmpͼƬλ,ɫ,1(26-27ֽ)
	unsigned short  biBitCount;      // һռλ,һΪ24(28-29ֽ)
	unsigned int    biCompression;   // ͼѹ,ѹ:0(30-33ֽ)
	unsigned int    biSizeImage;     // ռС,bfSize-bfOffBits(34-37ֽ)
	unsigned int    biXPelsPerMeter; // ˮƽֱ,/,һΪ0(38-41ֽ)
	unsigned int    biYPelsPerMeter; // ֱֱ,/,һΪ0(42-45ֽ)
	unsigned int    biClrUsed;       // λͼʵʹòɫеɫ,ʹеɫ:0(46-49ֽ)
	unsigned int    biClrImportant;  // ͼʾҪӰɫĿ,Ҫ:0(50-53ֽ)
} BITMAPINFOHEADER;

/* ɫ */
typedef struct RGBQuad
{
	unsigned char rgbBlue;		// ɫɫ,0-255
	unsigned char rgbGreen;		// ɫɫ,0-255
	unsigned char rgbRed;		// ɫĺɫ,0-255
...
char* Read_bmp(char* filepath, BITMAPFILEHEADER* bmf, BITMAPINFOHEADER* bmi)
{
	...
	fseek(fp, bmf->bfOffBits, SEEK_SET);	// ƶݿʼλ
...
int main(int argc, char** argv)
{
	...
	/* B,G,RȡΪά */
	unsigned int** B = (unsigned int**)malloc(h * sizeof(unsigned int*));	// ΪB,G,Rռ(ά)
	...
	double* table = (double*)malloc(6 * 256 * sizeof(double));	// Ϊtableڴ(6*256ά)
	if (!table) {
		printf("ڴʧܣ\n");
		return 0;
	}
	...
	double startTime = omp_get_wtime();	// ʼʱ
	printf("start: %f\n", startTime);
...
#pragma omp parallel for num_threads(4) private(i,j)
	...
	double endTime = omp_get_wtime();	// ֹͣʱ
	printf("end: %f\n", endTime);
	printf("вʱ: %15.15f\n", endTime - startTime);
