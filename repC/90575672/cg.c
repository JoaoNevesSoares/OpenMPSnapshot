...
int main(int argc, char *argv[])
{
  ...
  //---------------------------------------------------------------------
      /*ccccc #pragma omp parallel default(shared) private(i,j,k,zeta) 
     {*/
  //---------------------------------------------------------------------
  //modified: Removed the Pragma                             
  //---------------------------------------------------------------------
    tran    = 314159265.0;
    amult   = 1220703125.0;
...
//  }
 ...
 for (it = 1; it <= 1; it++) {
    ...
    //---------------------------------------------------------------------
   //---------------------------------------------------------------------
    //****** Modified : Added pragma omp single for norm_temp1,norm_temp2
    //--------------------------------------------------------------------- 
   #pragma omp single
      {   
            norm_temp1 = 0.0;
    ...
    //---------------------------------------------------------------------
    // ******Modified : remove norm_temp1,norm_temp2
    //---------------------------------------------------------------------    
    //norm_temp1 = 0.0;
    //norm_temp2 = 0.0;
    //---------------------------------------------------------------------
    // ******* Modified : Added pragma omp for only reduction and we are not parallelizing here.
    // so,remmove the parallel version.
    //--------------------------------------------------------------------- 
 ...
 #pragma omp for reduction(+:norm_temp1,norm_temp2)
    ...
    //---------------------------------------------------------------------
    //---------------------------------------------------------------------
    // ******* Modified : removed default(shared) private(j) to normalize z to obtain x
    //--------------------------------------------------------------------- 
    #pragma omp parallel for // ccccccc default(shared) private(j)
    for (j = 0; j < lastcol - firstcol + 1; j++) {     
      x[j] = norm_temp2 * z[j];
    }
  ...
  //---------------------------------------------------------------------
  // ******* Modified : removed pragma omp parallel for default(shared) private(i) 
  //--------------------------------------------------------------------- 
 // #pragma omp parallel for default(shared) private(i) //check
#pragma omp for nowait
  for (i = 0; i < NA+1; i++) {
    x[i] = 1.0;
  }
  ...
  //---------------------------------------------------------------------
   //---------------------------------------------------------------------
  // ******* Modified : removed #pragma omp parallel private(it,i,j,k) 
  //--------------------------------------------------------------------- 
  //#pragma omp parallel private(it,i,j,k)  
for (it = 1; it <= NITER; it++) {
    ...
    //---------------------------------------------------------------------
   //---------------------------------------------------------------------
  // ******* Modified : pragma omp single for  norm_temp1 ,norm_temp2
  //--------------------------------------------------------------------- 
 #pragma omp single
{
    norm_temp1 = 0.0;
  ...
  //---------------------------------------------------------------------
  // ******* Modified : removed default(shared) private(j)
  //--------------------------------------------------------------------- 
    #pragma omp parallel for /* ccc  default(shared) private(j)*/ \
                      ...
                      double *rnorm)
{
  ...
  //---------------------------------------------------------------------
  //***** Modified : Added #pragma omp single nowait for rho.
  //---------------------------------------------------------------------
#pragma omp single nowait
  ...
  //---------------------------------------------------------------------
  //---------------------------------------------------------------------
  //***** Modified : Added #pragma omp single nowait
  //---------------------------------------------------------------------
#pragma omp for nowait
  for (j = 0; j < naa+1; j++) {
    q[j] = 0.0;
...
#pragma omp for 
    ...
    */
    ...
    //---------------------------------------------------------------------
   //---------------------------------------------------------------------
  //***** Modified : Added reduction(+:d) to find d
  //---------------------------------------------------------------------    
...
//#pragma omp for
    ...
    //---------------------------------------------------------------------
    //---------------------------------------------------------------------
    //***** Modified : removed reduction(+:d) to find Z and r
    //---------------------------------------------------------------------   
    #pragma omp for //ccccc sreduction(+:rho)
    for (j = 0; j < lastcol - firstcol + 1; j++) {
      z[j] = z[j] + alpha*p[j];
      r[j] = r[j] - alpha*q[j];
    ...
    //---------------------------------------------------------------------
  //---------------------------------------------------------------------
  //***** Modified : removed #pragma omp single
  //---------------------------------------------------------------------  
//#pragma omp single    
 beta = rho / rho0;
 ...
 //---------------------------------------------------------------------
  //***** Modified : Added #pragma omp single
  //---------------------------------------------------------------------  
#pragma omp single
  {
  *rnorm = sqrt(sum);
                  ...
                  int iv[])
{
  ...
  //---------------------------------------------------------------------
  //***** Modified : Added #pragma omp parallel for vecset(routine conj_grad three 
