...
int main(int argc, char *argv[])
{
	...
	int myid, p; /* MPI μεταβλητές για να βλέπουμε το rank και size του process */
	int source; /* Μεταβληρή για να ορίζουμε την πηγή των μηνυμάτων */
	int tag = 1234; /* Μεταβληρή για να ορίσουμε το tag των μηνυμάτων */
	int dest = 0; /* Μεταβληρή για να ορίζουμε τον προορισμό των μηνυμάτων */
	...
	t0 = get_wtime();
	int provided;
	MPI_Init(&argc, &argv); /* Εκκίνηση διαμοιρασμού process */
	MPI_Comm_rank(MPI_COMM_WORLD, &myid); /* get current process id */
	MPI_Comm_size(MPI_COMM_WORLD, &p);	  /* get number of processes */
	srand48(time(0) * (myid + 1)); /* Εξασφαλίζουμε ότι σε κάθε Processes οι τιμές θα είναι όντως τυχαίες */
#pragma omp parallel num_threads(12) firstprivate(fx, jj, i) /*Εκκίνηση παραλληλισμού και δημιουργία νημάτων με την παράμετρο num_threads(int), επίσης χρησιμοποιούμε την μέθοδο firstprivate για να ορίσουμε τις μεταβλητές που δεν θέλουμε να διαμοιράζονται μεταξύ των threads*/
	{
#pragma omp for /*Παραλληλοποίηση βρόχου*/
		for (trial = 0; trial < ntrials / p; trial++) /* Χωρίζουμε τον αριθμό των trials σε κάθε process, αν δεν το κάναμε κάθε process Θα έκανε ntrials! */
		{
...
#pragma omp critical /*mutex περιοχή για τις μεταβλητές που μας επιστρέφουν το τελικό αποτέλεσμα, λ.χ. αν έχουμε 2 νέα best ταυτόχρονα θα υπάρξει πρόβλημα*/
	...
	if (myid != 0) /* Όλα τα processes που έχουν id διάφορο του 0 θα στείλουν τα αποτελέσματα τους στο master process με id = 0 */
	{
		MPI_Send(&best_fx, 1, MPI_DOUBLE, dest, tag, MPI_COMM_WORLD); /* Στέλνουμε τα δεδομένα των workers-processes στο master process */
	...
	{
		for (j = 1; j < p; j++) /* Για κάθε worker-process */
		{
			source = j;
			MPI_Recv(&fx, 1, MPI_DOUBLE, source, tag, MPI_COMM_WORLD, &status); /* Δεχόμαστε τα δεδομένα του κάθε process σειριακά */
			...
			if (fx < best_fx) /* Συγκρίνουμε αρχικά τα δεδομένα του master-process με το process με id = 1 και μετα σειριακά με τα υπόλοιπα */
			{
				best_trial = trial;
				best_jj = jj;
				best_fx = fx;
		...
		printf("Total number of function evaluations = %ld\n", funevals * p); /* Πολλαπλασιάζουμε τον αριθμό των πράξεων με τον αριθμό των processes για να βρούμε τον πραγματικό αριθμό των πράξεων */
