...
int main(int argc, char *argv[])
{
	...
	t0 = get_wtime();
	int num_threads = 12;
    #pragma omp parallel num_threads(num_threads)  /*Εκκίνηση παραλληλισμού και δημιουργία νημάτων με την παράμετρο num_threads(int), επίσης χρησιμοποιούμε την μέθοδο firstprivate για να ορίσουμε τις μεταβλητές που δεν θέλουμε να διαμοιράζονται μεταξύ των threads*/
    {
         #pragma omp single nowait /*Λέμε ότι το επόμενο μπλοκ κώδικο μπορεί να γίνει από οποιοδήποτε thread χωρίς να περιμένει να τελειώσουν τα άλλα (χρήσιμο σε loops χώρις ορίσμενο νούμερο επαναλήψεων)*/
            for (trial = 0; trial < ntrials; trial++) {
				#pragma omp task firstprivate(fx, jj, i) /*Εκκίνηση tasks για τον υπολογισμό της συνάρτησης*/
				{
	...
	printf("Total number of function evaluations = %ld\n", funevals * num_threads); /*Πολλαπλασιασμό με αριθμό threads για να βρούμε το πραγματικό αριθμό των πράξεων (αφού το αρχικό ποσό μοιράζεται σε n threads*/
