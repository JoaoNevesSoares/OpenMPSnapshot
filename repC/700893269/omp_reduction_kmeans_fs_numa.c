            ...
            double * clusters)         /* out: [numClusters][numCoords] */
{
    ...
    #define PADDING_INT (CACHE_LINE_SIZE / sizeof(int))
   ...
   for (k=0; k<nthreads; k++)
{
    ...
    // Original size + padding for every cluster (as it's a 2D array)
    ...
    do {
        ...
        #pragma omp parallel shared(local_newClusters,objects,delta,membership,clusters,local_newClusterSize,numCoords) private(i,j,index)
        {
        #pragma omp for
        for (i=0; i<numObjs; i++)
        {
