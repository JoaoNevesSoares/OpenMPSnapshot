...
{
    nodes_counter++;
    // MCTS_Node* rst = (MCTS_Node*)malloc(sizeof(MCTS_Node));
    // rst->expanded=false;
    // rst->parent=NULL;
...
{ 
    // move_t poss_moves[BUFFER_SIZE];
    while(!is_terminal(state))
    {
        // int n_moves = possible_moves_omp(state, poss_moves);
        // int m = (int)(n_moves*u_dist(gen));
        // apply_move(state, &poss_moves[m]);
