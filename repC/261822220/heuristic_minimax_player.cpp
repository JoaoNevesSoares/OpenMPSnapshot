...
//At each node for one state there are countless moves, we only explore the ones with high heristic values
//e.g. At layer 0, we explore 60 moves, then 40 at layer 1...
// const int EXP_B[7] = {60,40,25,20,15,10,5};
...
//Similar to heuristic_value.
//Want the heuristic_value if I apply this move onto this state
...
{
    //Explore the game to some certain depth
    init_depth = 6;
    //TODO: If still time, explore one more layer
    time_budget = 5.0;
...
//Each node returns to its parent its value, and the best move 
...
return_t HeuristicMinimaxPlayer::process(state_t* state, double alpha, double beta, int depth, int MAX_DEPTH)
{
    ...
    if(is_terminal(state))
    {
        //Give a value bigger than any heuristic values
    ...
    //For each move, construct an edge
