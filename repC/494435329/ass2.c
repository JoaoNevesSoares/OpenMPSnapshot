...
int main() {
    ...
    //getting number of records in the file
    while (!feof(record)) {
        ch = fgetc(record);
        if (ch == '\n') {
            count++;
    ...
    while (fgets(str, 50, record) != NULL) {
        ...
        points[indx].x = atoi(subtext + 1); //converting from char array to integer
    ...
    #pragma omp parallel num_threads(THREADS) shared(distance) private(i, j, k)
        ...
        #pragma omp parallel num_threads(THREADS) shared(clusters) private(j, mean)
        {
            ...
            // printf("mean of thread %d (%f , %f).\n", id, centroid[id].x, centroid[id].y);
        }
