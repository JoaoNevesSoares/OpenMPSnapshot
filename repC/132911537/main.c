...
void WalkMatrix(int direction, PuzzleMatrix* puzzle, char** list, int* i, WalkBundle* bundle, int** allNext, WordMesh* master) {
	...
	if(nextPos != -1) {
		if(IsPathContain(bundle, nextPos) == -1) {
			...
			//printf("if inside(walk) > currentCell = %d & holder = %s\n", bundle->currentCell, bundle->holder);
			bundle->pathLen++;
			...
			if(res == 0 || res == 1) {
				...
				WalkMatrix(1, puzzle, list, i, bundle, allNext, master); // East
				WalkMatrix(2, puzzle, list, i, bundle, allNext, master); // North East
				WalkMatrix(3, puzzle, list, i, bundle, allNext, master); // North
				WalkMatrix(4, puzzle, list, i, bundle, allNext, master); // North West
				WalkMatrix(5, puzzle, list, i, bundle, allNext, master); // West
				WalkMatrix(6, puzzle, list, i, bundle, allNext, master); // South West
				WalkMatrix(7, puzzle, list, i, bundle, allNext, master); // South
				WalkMatrix(8, puzzle, list, i, bundle, allNext, master); // South East
			}
...
char** GetStrings(PuzzleMatrix* puzzle, int* count, WordMesh* master, int startX, int startY) {
	...
	WalkMatrix(1, puzzle, stringList, &iterator, &bundle[0], allNext, master); // East
	WalkMatrix(2, puzzle, stringList, &iterator, &bundle[1], allNext, master); // North East
	WalkMatrix(3, puzzle, stringList, &iterator, &bundle[2], allNext, master); // North
	WalkMatrix(4, puzzle, stringList, &iterator, &bundle[3], allNext, master); // North West
	WalkMatrix(5, puzzle, stringList, &iterator, &bundle[4], allNext, master); // West
	WalkMatrix(6, puzzle, stringList, &iterator, &bundle[5], allNext, master); // South West
	WalkMatrix(7, puzzle, stringList, &iterator, &bundle[6], allNext, master); // South
	WalkMatrix(8, puzzle, stringList, &iterator, &bundle[7], allNext, master); // South East
