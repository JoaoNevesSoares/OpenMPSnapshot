...
typedef struct cell{
	...
	int mit_s, mov_s;
	float grown_rate;
	bool blocked;	//usado para simulacin con bloques
}cell;
...
int look_for_mitosis(int x, int y){
	// tomaremos en cuenta la posicin del teclado nmerico
	bool flag=false;
	int deep=1;
...
/*int check_food(int x, int y){
	...
	//se influye por la direccin de la comida
	for(int i=x_ini ; i<=x_end ; i++)
	...
	//consigue la direccin
	if(ddx==0 && ddy==0)
		s=5;
	else
		s=check_direction(ddx,ddy);
	//comprueba si se puede mover en dicha direccin
	bool move=false;
	int i=x,j=y;
	for(int ii=0;ii<1;ii++){	
		...
		if(move)
			return s;
		else{//ltima oportunidad de moverse
			s=(rand()%4+1)+5*(rand()%2);
			//printf("Next move: %d\n",s);
		}
	}
		...
		for(int i=0; i<long_x;i++){
			for(int j=0;j<long_y;j++){
				if(c[i][j].draw && !c[i][j].blocked){
					//clula envejece
					c[i][j].time_life++;
					//comprueba si est lo suficientemente grande para hacer mitosis
					...
					if(!c[i][j].mitosis){
						if(food[i][j]>0){
							//clula crece
							if(c[i][j].rad<radius*sqrt(2)){	//si la clula est muy grande ya no se alimenta tan rpido
							...
							c[i][j].time_life+=max_life/250;	//envejece mas rpido si no hay comida
...
//	printf("Tiempo de simulacin: %9.0lf ms with %d cells \n",(omp_get_wtime()-start)*1000,number_of_cells);
//	tiempo++;
	glutPostRedisplay();
