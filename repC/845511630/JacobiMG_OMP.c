//Parallel Jacobi Method with Multigrid using OpenMP
#include<stdio.h>
#include<stdlib.h>
#include<math.h>
#include<omp.h>
...
int main(){
    ...
    int N1 = 33, N2 = 17;
    int pre = 5; //Number of smoothing itereatons
    ...
    while(err>0.0001){
        ...
        #pragma omp parallel num_threads(thread_count) default(none) private(i,j,k) shared(pre,N1,phi_old,phi,delta1,q,res1,res2,N2,delx2_old,delx2,delta2,delx1,err,phi_exact)
        {
    ...
    printf("Number of iterations for Parallel Jacobi Method with Multigrid using OpenMP are : %d\n",2*pre*iter);
